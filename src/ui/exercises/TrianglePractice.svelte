...
<main class="exercise-page page">
  ...
  <section class="exercise-card" aria-labelledby="triangle-title">
    ...
    {#if exercise.answerMode === "select"}<div class="triangle-size-controls" aria-label="Chọn kích thước tam giác"><button class:active-size={activeSize === "small"} type="button" onclick={() => (activeSize = "small")}>Nhỏ</button>{#if definition?.validTriangles.some((item) => item.sizeClass === "medium")}<button class:active-size={activeSize === "medium"} type="button" onclick={() => (activeSize = "medium")}>Vừa</button>{/if}{#if definition?.validTriangles.some((item) => item.sizeClass === "large")}<button class:active-size={activeSize === "large"} type="button" onclick={() => (activeSize = "large")}>Lớn</button>{/if}</div><p class="triangle-count">Con đã tìm: <strong>{selectedIds.length}</strong></p>{/if}
