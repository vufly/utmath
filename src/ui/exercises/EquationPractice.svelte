        ...
        {#if hint.type === "visual" && numberLineStart !== undefined && numberLineEnd !== undefined}
          ...
          <div class="strategy-number-line" aria-label={`Từ ${numberLineStart} đến ${numberLineEnd}, ${numberLineSteps} bước trên dãy số`}>
            {#each Array(11) as _, value}
              <span class:line-start={value === numberLineStart} class:line-step={value > Math.min(numberLineStart, numberLineEnd) && value < Math.max(numberLineStart, numberLineEnd)}>{value}{#if (numberLineEnd > numberLineStart && value >= numberLineStart && value < numberLineEnd) || (numberLineEnd < numberLineStart && value <= numberLineStart && value > numberLineEnd)}<svg class:backward={numberLineEnd < numberLineStart} class="step-arc" aria-hidden="true" viewBox="0 0 40 18"><path d="M2 5C12 16 26 16 36 5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-width="2" /><path d="m29.2 7.2 6.8-2.2-1.6 6.8" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" /></svg>{/if}</span>
            {/each}
          </div>
        {/if}
        {#if missingVisual}
