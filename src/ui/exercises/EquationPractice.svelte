...
<main class="exercise-page page">
  ...
  <section class="exercise-card equation-card" aria-labelledby="equation-title">
    ...
    {#if hint}<aside class="hint-card" aria-live="polite"><strong>Gợi ý</strong><p>{typeof hint.payload === "string" ? hint.payload : "Con thử nhìn mối liên hệ giữa các số nhé."}</p>{#if hint.type === "visual" && numberLineStart !== undefined && numberLineEnd !== undefined}<div class="strategy-number-line" aria-label={`Từ ${numberLineStart} đến ${numberLineEnd}`}>{#each Array(11) as _, value}<span class:line-start={value === numberLineStart} class:line-end={value === numberLineEnd} class:line-step={value > Math.min(numberLineStart, numberLineEnd) && value < Math.max(numberLineStart, numberLineEnd)}>{value}</span>{/each}</div>{/if}{#if missingVisual}<div class="missing-number-frame" aria-label="Sơ đồ phần và cả số">{#each Array(missingVisual.total) as _, index}<span class:known-counter={index < missingVisual.known} class:split-counter={index === missingVisual.split}><i></i></span>{/each}</div>{/if}</aside>{/if}
